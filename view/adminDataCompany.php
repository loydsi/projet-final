<section class="admin__data__company">

    <a href="index.php?route=company" class="arrow"><i class="fa-solid fa-arrow-left"></i></a>

    <h2>Nom entreprise</h2>

    <table class="admin__data__company__list">
        <tr>
            <td>Nom de L'entreprise</td>
            <td>Leyila's Agency</td>
        </tr>
        <tr>
            <td>Email</td>
            <td>loyd33@hotmail.fr</td>
        </tr>
        <tr>
            <td>Telephone</td>
            <td>06 67 66 59 61</td>
        </tr>
        <tr>
            <td>Siret</td>
            <td>123 568 941 00056</td>
        </tr>
        <tr>
            <td>Adresse</td>
            <td>3, rue Florence Arthaud</td>
        </tr>
        <tr>
            <td>Code Postal</td>
            <td>33140</td>
        </tr>
        <tr>
            <td>Ville</td>
            <td>Villenave d'Ornon</td>
        </tr>
    </table>

    <table class="admin__data__company__facture">
        <tr>
            <td>factures 1</td>
            <td>telecharger</td>
        </tr>
    </table>

    <table class="admin__data__company__devis">
        <tr>
            <td>Devis 1</td>
            <td>telecharger</td>
        </tr>
    </table>
    <a href="#" class="admin__data__company__button">Supprimer profil</a>
    <a href="index.php?route=companyupdate" class="admin__data__company__button">Modifier profil</a>
</section>